...
import { citiesToCategories, LocationCategory } from '@/utils/locationCategories';

export const useVenueLocationCategories = () => {
  return useQuery({
    queryKey: ['venue-location-categories'],
    queryFn: async (): Promise<LocationCategory[]> => {
      console.log('Fetching venue location categories...');
      ...
      const categories = citiesToCategories(uniqueCities);
