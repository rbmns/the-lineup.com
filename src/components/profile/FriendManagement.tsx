...
export const FriendManagement: React.FC<FriendManagementProps> = ({
  ...
  const checkFriendshipStatus = async (userId: string, profileId: string) => {
    try {
      ...
      const {
        data: sentRequest,
        error: sentError
      } = await supabase.from('friend_requests').select('*').eq('sender_id', userId).eq('receiver_id', profileId).single();
      ...
      const {
        ...
        error: receivedError
      } = await supabase.from('friend_requests').select('*').eq('sender_id', profileId).eq('receiver_id', userId).single();
      ...
      } = await supabase.from('friends').select('*').or(`user_id.eq.${userId}, friend_id.eq.${userId}`).or(`user_id.eq.${profileId}, friend_id.eq.${profileId}`);
      if (friendshipError) {
        console.error('Error checking friendship:', friendshipError);
      } else {
        const areFriends = friendship && friendship.some(record => record.user_id === userId && record.friend_id === profileId || record.user_id === profileId && record.friend_id === userId);
        if (areFriends) {
  ...
  const removeFriendship = async (userId: string, friendId: string) => {
    try {
      ...
      const {
        data,
        error: findError
      } = await supabase.from('friendships').select('id').or(`and(user_id.eq.${userId},friend_id.eq.${friendId}),and(user_id.eq.${friendId},friend_id.eq.${userId})`).eq('status', 'Accepted').maybeSingle();
      if (findError) {
  ...
  const handleAcceptRequest = async () => {
    ...
    try {
      ...
      const {
        data,
        error: findError
      } = await supabase.from('friendships').select('id').eq('user_id', profile.id).eq('friend_id', currentUserId).eq('status', 'Pending').maybeSingle();
  ...
  return <>
      {friendshipStatus === 'none' && <Button onClick={handleAddFriend} disabled={isProcessing} className="mt-4 text-white bg-cyan-900 hover:bg-cyan-800">
          {isProcessing ? 'Processing...' : 'Add Friend'}
        </Button>}
      
      {friendshipStatus === 'pending' && <Button disabled className="mt-4 bg-gray-400 text-white cursor-not-allowed">
          Request Pending
        </Button>}
      
      {friendshipStatus === 'accepted' && <Button onClick={handleRemoveFriend} variant="outline" className="mt-4 border-red-500 text-black hover:bg-red-50 bg-white" disabled={isProcessing}>
      ...
      <AlertDialog open={showUnfriendDialog} onOpenChange={setShowUnfriendDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmUnfriend} className="bg-white border border-red-500 text-black hover:bg-red-50" disabled={isProcessing}>
