    ...
    <div className="space-y-8">
      ...
      <Card className="bg-pure-white border border-mist-grey shadow-sm">
        <CardHeader className="pb-4">
          <CardTitle className="text-h4 font-montserrat text-graphite-grey flex items-center">
        ...
        <CardContent className="space-y-4">
          {event.venues ? (
            <>
              {event.venues.name && (
                <div>
                  <h4 className="font-semibold text-graphite-grey font-montserrat text-base">{event.venues.name}</h4>
                </div>
              )}
              
              <div className="text-body-small text-graphite-grey/70 font-lato">
                {[
                  event.venues.street,
              ...
              {googleMapsUrl && (
                <a 
                  ...
                  className="inline-flex items-center gap-2 text-ocean-teal hover:text-ocean-teal/80 transition-colors text-body-small font-medium font-lato mt-3"
                >
                  View on map
                </a>
              )}
            ...
            <div className="text-body-small text-graphite-grey/60 italic font-lato">
      ...
      {hasBookingInfo && (
        <Card className="bg-pure-white border border-mist-grey shadow-sm">
          <CardHeader className="pb-4">
            <CardTitle className="text-h4 font-montserrat text-graphite-grey">
              Booking Info
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            {event.fee !== null && event.fee !== undefined && (
              <div>
                <p className="text-body-small font-medium text-graphite-grey mb-1 font-lato">Entry fee</p>
                <p className="text-base font-semibold text-graphite-grey font-montserrat">
              ...
              <>
                ...
                <div>
                  <a 
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 bg-ocean-teal text-pure-white px-4 py-2 rounded-md hover:bg-ocean-teal/90 transition-colors font-medium text-body-small font-montserrat"
                  >
        ...
        <Card className="bg-pure-white border border-mist-grey shadow-sm">
          ...
          <CardContent className="space-y-6">
            {friendAttendees.going.length > 0 && (
              <div>
                <h4 className="text-base font-medium text-graphite-grey mb-3 flex items-center font-montserrat">
                  Going
                  <span className="ml-2 text-body-small text-graphite-grey/70 font-lato">
                  ...
                  {friendAttendees.going.slice(0, 3).map(attendee => (
                    ...
                    >
                      <div className="w-8 h-8 bg-gradient-to-r from-ocean-teal to-sage rounded-full flex items-center justify-center shadow-sm">
                        <span className="text-xs font-bold text-pure-white font-montserrat">
                          {attendee.username?.charAt(0).toUpperCase() || 'U'}
                        </span>
                      </div>
                      <span className="text-body-small font-medium text-graphite-grey font-lato">{attendee.username}</span>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="pt-4 border-t border-mist-grey">
              <h4 className="text-base font-medium text-graphite-grey mb-2 font-montserrat">All Attendees</h4>
              <div className="flex items-center gap-4 text-body-small font-lato">
