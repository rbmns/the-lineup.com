...
export const EventDetailMainContent: React.FC<EventDetailMainContentProps> = ({
  ...
  useEffect(() => {
    const filterAttendees = async () => {
      ...
      try {
        const [friendsGoing, friendsInterested] = await Promise.all([filterFriendsFromAttendees(attendees.going || [], user.id), filterFriendsFromAttendees(attendees.interested || [], user.id)]);
        setFriendAttendees({
          going: friendsGoing,
  ...
  return <div className="lg:col-span-2 space-y-8">
      ...
      {isAuthenticated && <div className={`transition-all duration-300 ${rsvpFeedback ? 'scale-105' : ''} ${rsvpFeedback === 'going' ? 'ring-2 ring-emerald-200 ring-opacity-50' : rsvpFeedback === 'interested' ? 'ring-2 ring-sky-200 ring-opacity-50' : ''}`}>
      ...
      <div className="text-left bg-gradient-to-r from-sage/5 to-clay/5 rounded-xl p-6 border border-sage/10">
        ...
        <div className="space-y-2">
          {event.venues?.name && <p className="font-semibold text-midnight font-lato text-base">{event.venues.name}</p>}
      ...
      {(event.booking_link || event.fee !== null && event.fee !== undefined) && <div className="text-left bg-gradient-to-r from-clay/5 to-coral/5 rounded-xl p-6 border border-clay/10">
          ...
          <div className="space-y-4">
            {event.fee !== null && event.fee !== undefined && <div className="flex items-center gap-3">
            ...
            {event.booking_link && <div>
                ...
                <a href={event.booking_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 bg-clay text-coconut px-4 py-2 rounded-md hover:bg-clay/90 transition-colors font-montserrat font-medium text-sm">
      ...
      <div className="text-left bg-gradient-to-r from-sage/10 to-clay/10 rounded-xl p-6 border border-sage/20">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-gradient-to-r from-sage to-clay rounded-full flex items-center justify-center shadow-md">
              <span className="text-lg font-bold text-coconut font-montserrat">
                {event.organiser_name ? event.organiser_name.charAt(0).toUpperCase() : event.creator?.username ? event.creator.username.charAt(0).toUpperCase() : 'H'}
              </span>
            </div>
            <div className="flex-1">
              ...
              {event.creator?.email && <p className="font-mono text-xs text-overcast">
                  {event.creator.email}
                </p>}
            </div>
          </div>
          ...
          <div className="space-y-3">
            {event.organizer_link && <div>
                <p className="font-lato text-sm text-overcast mb-1">Organizer website</p>
                <a href={event.organizer_link} target="_blank" rel="noopener noreferrer" className="text-sm text-clay hover:text-midnight hover:underline transition-colors font-mono break-all">
      ...
      {event.extra_info && <div className="text-left bg-ivory/50 rounded-xl p-6 border border-sage/10">
      ...
      {isAuthenticated && (friendAttendees.going.length > 0 || friendAttendees.interested.length > 0) && <div className="text-left bg-gradient-to-r from-sage/10 to-clay/10 rounded-xl p-6 border border-sage/20">
          <h3 className="text-lg font-semibold mb-4 text-left tracking-tight text-midnight font-montserrat flex items-center">
          ...
          <div className="space-y-6">
            {friendAttendees.going.length > 0 && <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {friendAttendees.going.map(attendee => <button key={attendee.id} onClick={() => handleUserClick(attendee.id)} className="flex items-center gap-2 bg-ivory/80 backdrop-blur-sm rounded-full px-4 py-2 hover:bg-coconut hover:shadow-md transition-all duration-200 cursor-pointer border border-overcast/20">
            ...
            {friendAttendees.interested.length > 0 && <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {friendAttendees.interested.map(attendee => <button key={attendee.id} onClick={() => handleUserClick(attendee.id)} className="flex items-center gap-2 bg-ivory/80 backdrop-blur-sm rounded-full px-4 py-2 hover:bg-coconut hover:shadow-md transition-all duration-200 cursor-pointer border border-overcast/20">
