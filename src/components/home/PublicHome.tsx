...
export const PublicHome: React.FC<PublicHomeProps> = ({
  ...
  const processEventData = (event: any): Event => {
    ...
    return {
      ...
      rsvp_status: user 
        ? (event.event_rsvps?.find((rsvp: any) => rsvp.user_id === user.id)?.status as 'Interested' | 'Going' | undefined)
        : undefined,
      area: null,
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      if (selectedEventTypes.length > 0) {
        query = query.in('event_type', selectedEventTypes as any[]);
      }
      ...
      if (data && data.length > 0) {
        ...
        const upcomingEvents = filterUpcomingEvents(formattedEvents);
        setCombinedResults(upcomingEvents);
      } else {
        setCombinedResults([]);
      }
    } catch (error) {
  ...
  if (combinedResults && combinedResults.length > 0) {
    displayEvents = combinedResults;
  } else if (similarEvents.length > 0 && noResultsFound) {
    displayEvents = similarEvents;
  } else if (!searchQuery && !selectedEventTypes.length) {
    displayEvents = filterUpcomingEvents(events || []);
  } else {
    displayEvents = [];
  }
